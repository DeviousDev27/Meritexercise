...
public class Exercise05_Weather {
    ...
    */
    public int hottestDay(int[] dailyHighs) {
        int hottestDayValue = dailyHighs[0];

        for (int i = 0; i < dailyHighs.length; i++){
            if(dailyHighs[i] > hottestDayValue){
                hottestDayValue = dailyHighs[i];

            }
        }
        return hottestDayValue;
    }
    ...
    public int[] fixTemperatures(int[] temperatures) {
        int [] afterChanges = new int[temperatures.length];

        for(int i = 0; i < afterChanges.length; i ++){
            if(i%2 == 0) {
                afterChanges[i] = temperatures[i] + 2;

                temperatures[i] = afterChanges[i];
