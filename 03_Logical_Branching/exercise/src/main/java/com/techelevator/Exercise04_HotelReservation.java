...
public class Exercise04_HotelReservation {
    ...
    public double calculateStayTotal(int numberOfNights) {
        ...
        if (numberOfNights < MINIMUM_NIGHTS_FOR_DISCOUNT_RATE){
            totalPrice = numberOfNights * DAILY_RATE;
        }else {
            totalPrice = numberOfNights * DISCOUNT_RATE;
        }
        return totalPrice;
    }
    ...
    public double calculateStayTotal(int numOfTotalNights, int numOfWeekendNights) {
        ...
        if (numOfTotalNights < MINIMUM_NIGHTS_FOR_DISCOUNT_RATE){
            totalPrice = numOfTotalNights * DAILY_RATE;
        }else {
            double weekdayNights = numOfTotalNights - numOfWeekendNights;
            totalPrice = weekdayNights * DISCOUNT_RATE;
            if(numOfWeekendNights >= 1){
                double wnkTotal = numOfWeekendNights * DAILY_RATE;
                double completeTotal = wnkTotal + totalPrice;
    ...
    public double calculateStayTotal(int numOfTotalNights, int numOfWeekendNights, boolean isRewardsMember) {
        if (numOfTotalNights < MINIMUM_NIGHTS_FOR_DISCOUNT_RATE) {
            double shortStayPrice = numOfTotalNights * DAILY_RATE;
            if(isRewardsMember){
                double rewardShortDay = numOfTotalNights * DISCOUNT_RATE;
                return rewardShortDay;
            }else {
                return shortStayPrice;
            }
        } else {
            double weekdayNights = numOfTotalNights - numOfWeekendNights;
            double longStayPrice = weekdayNights * DISCOUNT_RATE;
            if (numOfWeekendNights > 0) {
                double wnkTotal = numOfWeekendNights * DAILY_RATE;
                double completeTotal = wnkTotal + longStayPrice;

                if(isRewardsMember){
                    double rewardLongDay = numOfTotalNights * DISCOUNT_RATE;
                    return rewardLongDay;
                }else {
                    return completeTotal;
                }
            }else {
                return longStayPrice;
            }
