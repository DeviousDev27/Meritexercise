...
public class Exercise03_ShippingTotal {
    ...
    public double calculateShippingTotal(int weightPounds) {
        ...
        if (weightPounds <= MAX_WEIGHT_POUNDS){
            double totalPrice = weightPounds * UP_TO_40_LB_RATE;
            return totalPrice;
        }else {
            double weightPrice = MAX_WEIGHT_POUNDS * UP_TO_40_LB_RATE;
            double additionalWeight = weightPounds - MAX_WEIGHT_POUNDS;
            double additionalPrice = additionalWeight * OVER_40_LB_RATE;
            double totalPrice = additionalPrice + weightPrice;
            return totalPrice;
    ...
    public double calculateShippingTotal(int weightPounds, boolean hasDiscount) {
        ...
        if (weightPounds <= MAX_WEIGHT_POUNDS){
            double totalPrice = weightPounds * UP_TO_40_LB_RATE;
            if(hasDiscount){
                double afterDiscount = totalPrice - (totalPrice * discountCode);
    ...
    public double calculateShippingTotal(int weightPounds, double discountPercentage) {
        if (weightPounds <= MAX_WEIGHT_POUNDS){
            double totalPrice = weightPounds * UP_TO_40_LB_RATE;
                double afterDiscount = totalPrice - (totalPrice * discountPercentage);
                return afterDiscount;
            } else{
            double weightPrice = MAX_WEIGHT_POUNDS * UP_TO_40_LB_RATE;
            double additionalWeight = weightPounds - MAX_WEIGHT_POUNDS;
