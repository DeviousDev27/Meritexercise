...
public class Exercise06_ElectricBill {
    ...
    public double calculateElectricBill(double unitsUsed) {
        ...
        if(unitsUsed <= BASIC_SERVICE_LIMIT){
            double serviceRate = unitsUsed * BASIC_SERVICE_RATE;
            return serviceRate;
        }else{
            double BasicRate = BASIC_SERVICE_RATE * BASIC_SERVICE_LIMIT;
            double newUnitsUsed = unitsUsed - BASIC_SERVICE_LIMIT;
            double excessRate = newUnitsUsed * EXCESS_SERVICE_RATE;
            double newBillRate = BasicRate + excessRate;
            return newBillRate;
        }
    }
    ...
    public double calculateElectricBill(double unitsUsed, boolean hasRenewableEnergy) {
        if(unitsUsed <= BASIC_SERVICE_LIMIT){
            ...
            if(hasRenewableEnergy){
                double afterDiscount = serviceRate - (serviceRate * RENEWABLE_ENERGY_DISCOUNT);
            ...
            double newBillRate = basicRate + excessRate;
            if (hasRenewableEnergy) {
                double afterDiscount = newBillRate - (newBillRate * RENEWABLE_ENERGY_DISCOUNT);
                return afterDiscount;
            } else {
                return newBillRate;
            }
    ...
    public double calculateElectricBill(double unitsUsed, double unitsReturned) {
        ...
        if(netUsage <= BASIC_SERVICE_LIMIT){
            double serviceRate = netUsage * BASIC_SERVICE_RATE;
            if(unitsReturned > 0 && serviceRate > 0){
                double afterDiscount = serviceRate - (serviceRate * RENEWABLE_ENERGY_DISCOUNT);
                return afterDiscount;
            }else{
                return serviceRate;
            ...
            double basicRate = BASIC_SERVICE_RATE * BASIC_SERVICE_LIMIT;
            double newUnitsUsed = netUsage - BASIC_SERVICE_LIMIT;
            double excessRate = newUnitsUsed * EXCESS_SERVICE_RATE;
            double newBillRate = basicRate + excessRate;
            if (unitsReturned > 0 && newBillRate > 0) {
                double afterDiscount = newBillRate - (newBillRate * RENEWABLE_ENERGY_DISCOUNT);
                return afterDiscount;
            } else {
                return newBillRate;
